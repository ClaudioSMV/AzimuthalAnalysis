...
try:
    ...
    print("  [SummaryRatio] Summary only needs two numbers in dataset, no Target nor underscore")
...
# par_y_lmts = [[0.0,2.0], [-1.0,3.0]] if not usePt2 else [[0.0,2.0], [-2.0,4.0]]
par_y_lmts = [[0.201,1.799], [0.001,1.999]] if not usePt2 else [[0.301, 1.299], [0.001,1.999]]
...
for r,typeR in enumerate(type_reco_short):
    ...
    for p,par in enumerate(["B", "C"]):
        ...
        ratio1L.SetLineColorAlpha(ROOT.kRed, 0.5)
...
for t,targ in enumerate(list_targets):
    ...
    this_hist = list_type_reco[0][0][t][iQ][iN]

    # if (iQ!=0 and iN!=2): this_hist.GetYaxis().SetNoExponent(ROOT.kTRUE)

    this_hist.SetMinimum(par_y_lmts[0][0])
    this_hist.SetMaximum(par_y_lmts[0][1])
    this_hist.SetLabelSize(tsize-20,"xy")
    this_hist.SetTitleSize(tsize-16,"xy")
    this_hist.SetTitleOffset(1.0,"x")
    this_hist.SetTitleOffset(1.8,"y")
...
# canvas_new.cd(0)
ms.draw_summary("%s ratio"%(fancy_uptitle[0]))
ms.draw_targetinfo("Solid targets", "Data")

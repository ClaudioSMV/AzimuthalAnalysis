...
for p,par in enumerate(["B", "C"]):
    ...
    mS.draw_preliminary("%s ratio Solid/D"%(par))
    ...
    this_canvas.cd(0)
    this_title_gif = mS.get_summary_fullpath("Par%s_Ratio%s"%(par,dataset_title), "gif", plots_cuts, isJLab, "Thesis/"+dataset_title[1:])
    if ("LR" in this_title_gif):
        this_title_gif = mS.add_str_before_ext(this_title_gif, "-%s"%(fit), "gif")

    this_title_pdf = mS.get_summary_fullpath("Par%s_Ratio%s"%(par,dataset_title), "pdf", plots_cuts, isJLab, "Thesis/"+dataset_title[1:])
